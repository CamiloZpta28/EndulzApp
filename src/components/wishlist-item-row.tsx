...
export function WishlistItemRow({
  ...
  groupId: string;
  editable: boolean;
  /** Cuando viene, la fila muestra las flechas para cambiar la prioridad. */
  priority?: { ids: string[]; index: number; memberId: string };
}) {
    ...
    return (
      <li className="bg-card rounded-xl border p-3">
        <form action={updateAction} className="space-y-3">
          ...
          {/* Acá el aviso de "esto son varios" es el que más sirve: los
              antojos apelmazados de antes lo ven al abrir el editor. */}
          <ItemNameField
